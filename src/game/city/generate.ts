...
export function generateCity(seed: number): City {
  ...
  // Local streets and blocks for whichever districts have graduated to real
  // ones (#268, ADR-0009): grown around that district's own major roads
  // rather than a synthetic grid, one area at a time. `waterfront` (Ashford
  // Point) is the pilot; everything else stays without blocks until it earns
  // its own pass.
  for (const region of PLAN_DISTRICTS) {
    if (!CITY_LOCAL_STREETS_KINDS.includes(region.kind)) continue;
    const grown = localStreetsFor(region, laid, router, water, land, centre(bounds), rng);
    laid.push(...grown.spans);
    blocks.push(...grown.blocks);
  }
